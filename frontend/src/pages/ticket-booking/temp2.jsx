...
export default function EventCard({ event, className }) {
  ...
  return (
    <Link to={`/ticketbooking/${event._id}`} className={cn("block", className)}>
      <Card className="overflow-hidden w-full max-w-[340px] py-0 h-full rounded-xl shadow-md hover:shadow-xl transition-all duration-300 border-0">
        ...
        <div className="relative h-[220px] overflow-hidden">
          ...
          <Badge className="absolute top-3 left-3 border-0" variant="secondary">
            {event.title || "SPORTS"}
          </Badge>
        </div>
